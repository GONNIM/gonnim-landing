// Node-runtime Freemoa crawler → Supabase.
// Run locally: pnpm exec tsx scripts/crawl-freemoa.ts
...
import { FreemoaCrawler } from "../src/lib/crawlers/freemoa";
...
async function main() {
  ...
  const crawler = new FreemoaCrawler();
  ...
  console.log(
    `freemoa · ${status} · found=${result.projects.length} new=${newProjects} updated=${updatedProjects}`,
  );
