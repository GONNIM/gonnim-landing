// Claude-powered draft proposal generator.
// Uses prompt caching (Anthropic 2024+): user profile is a cached block
// so subsequent calls are ~90% cheaper. Model default: claude-sonnet-4-6.
//
// Env: ANTHROPIC_API_KEY (required · absent → 503 throw)
...
반드시 다음 JSON 형식으로만 응답하세요. 다른 설명·마크다운 없이 JSON만 반환:
{
  ...
  "suggested_budget": 500만원_단위_숫자_또는_null,
...
export async function generateDraft(project: ProjectContext): Promise<DraftResult> {
  ...
  if (!apiKey) {
    const err = new Error("ANTHROPIC_API_KEY missing · Vercel/로컬 env 등록 필요");
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `다음 프로젝트에 지원하는 제안서 초안을 위 JSON 형식으로 작성하세요.\n\n${projectBlock}`,
      },
    ],
  });
  ...
  if (!textBlock || textBlock.type !== "text") {
    throw new Error("Claude 응답에서 텍스트 블록을 찾을 수 없음");
  }
...
function extractJsonPayload(raw: string): string {
  // Claude sometimes wraps JSON in ```json blocks — strip if present
